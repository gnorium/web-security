...
internal enum Blake2b {
  ...
  static func hash<D: DataProtocol>(data: D, outLength: Int) -> Data {
    ...
    for i in 0..<blockCount {
      ...
      for j in 0..<16 {
        ...
        for k in 0..<8 {
          ...
          if idx < buffer.count { val |= UInt64(buffer[idx]) << (k * 8) }
        }
        block[j] = val
      }
    ...
    for i in 0..<((outLength + 7) / 8) {
      var val = h[i]
      for _ in 0..<min(8, outLength - i * 8) {
  ...
  private static func mixStep(
    v: inout [UInt64], a: Int, b: Int, c: Int, d: Int, x: UInt64, y: UInt64
  ) {
    v[a] = v[a] &+ v[b] &+ x
