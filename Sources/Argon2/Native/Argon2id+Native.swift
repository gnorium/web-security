...
internal enum Argon2NativeImplementation {
    ...
    for pass in 0..<t {
      for slice in 0..<4 {
        ...
        var generators: [IndexGenerator?] = (0..<p).map { lane in
          if variant == .i || (variant == .id && pass == 0 && slice < 2) {
            return IndexGenerator(
              pass: pass, lane: lane, slice: slice, m_prime: m_prime, iterations: t,
              variant: variant)
          }
          return nil
        }
        for col in 0..<sliceLen {
          ...
          for lane in 0..<p {
            let (l, z) = computeIndices(
              pass: pass, lane: lane, slice: slice, col: col, sliceLen: sliceLen, p: p, q: q,
              variant: variant, blocks: blocks, generator: &generators[lane])
            let prevCol = (j == 0 ? q - 1 : j - 1)
            let nextBlock = g(x: blocks[lane * q + prevCol], y: blocks[l * q + z])
            if pass == 0 {
              blocks[lane * q + j] = nextBlock
            } else {
              blocks[lane * q + j].xor(with: nextBlock)
            }
          }
        }
      }
  ...
  private static func gb(_ a: inout UInt64, _ b: inout UInt64, _ c: inout UInt64, _ d: inout UInt64)
  {
    func f(_ x: UInt64, _ y: UInt64) -> UInt64 {
      let x32 = x & 0xFFFF_FFFF
      let y32 = y & 0xFFFF_FFFF
  ...
  private static func rotateRight(_ value: UInt64, by: Int) -> UInt64 {
  ...
  private static func computeIndices(
    pass: Int, lane: Int, slice: Int, col: Int, sliceLen: Int, p: Int, q: Int, variant: Variant,
    blocks: [Block], generator: inout IndexGenerator?
  ) -> (Int, Int) {
    var j1: UInt32 = 0
    var j2: UInt32 = 0
    if generator != nil {
      ...
      j1 = UInt32(v0 & 0xFFFF_FFFF)
      j2 = UInt32(v0 >> 32)
    }
    let l = (pass == 0 && slice == 0) ? lane : Int(j2 % UInt32(p))
    var refSize: Int
    if pass == 0 {
      refSize = (l == lane) ? (slice * sliceLen + col - 1) : (slice * sliceLen - (col == 0 ? 1 : 0))
    } else {
      refSize = (l == lane) ? (q - sliceLen + col - 1) : (q - sliceLen - (col == 0 ? 1 : 0))
    }
  ...
  private static func blockToData(_ block: Block) -> Data {
    var data = Data()
    for i in 0..<128 {
      withUnsafeBytes(of: block.v[i].littleEndian) { data.append(contentsOf: $0) }
    }
    return data
  }
}
