...
public struct TOTPAuthenticator: Sendable {
  ...
  public func generateOTPAuthURL(
    ...
    let encodedIssuer =
      issuer.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? issuer
    let encodedAccount =
      accountName.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? accountName
    return
      "otpauth://totp/\(encodedIssuer):\(encodedAccount)?secret=\(secret)&issuer=\(encodedIssuer)&algorithm=\(algorithm)&digits=\(digits)&period=\(Int(timeStep))"
  }
  ...
  private func generateCode(secret: String, counter: Int) throws -> String {
    ...
    let truncatedHash =
      ((UInt32(hmacBytes[offset]) & 0x7f) << 24 | (UInt32(hmacBytes[offset + 1]) & 0xff) << 16
        | (UInt32(hmacBytes[offset + 2]) & 0xff) << 8 | (UInt32(hmacBytes[offset + 3]) & 0xff))
